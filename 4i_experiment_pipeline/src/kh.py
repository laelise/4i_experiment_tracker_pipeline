...
def kernel_herding_main(sample_set_ind,
    ...
    """
    X = X[sample_set_ind, :]
    logging.warning(f'Failpoint Test 2.1')
    phi = random_feats(X, gamma = gamma, frequency_seed = frequency_seed)
    logging.warning(f'Failpoint Test 2.2')
    kh_indices = kernel_herding(phi, num_subsamples)
    logging.warning(f'Failpoint Test 2.3')
...
def sketch(adata,
    ...
    logging.warning(f'Cleared job assignment')
    ...
    logging.warning(f'Cleared sample key setting')
    ...
    logging.warning(f'Cleared Min cell size detection')
        
    n_sample_sets = len(sample_set_inds)
    X = _parse_input(adata)
    logging.warning(f'Failpoint Test 1')

    p = mp.Pool(n_jobs)
    logging.warning(f'Failpoint Test 2')
    ...
    logging.warning(f'Failpoint Test 3')
    ...
    logging.warning(f'Failpoint Test 4')

    adata_subsample = anndata.concat(adata_subsample)
    logging.warning(f'Failpoint Test 5')
